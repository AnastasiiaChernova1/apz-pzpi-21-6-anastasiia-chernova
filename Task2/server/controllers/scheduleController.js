...
class ScheduleController {
    ...
    async create(req, res, next) {
        try {
            const { movieId, hallId, startTime, endTime, date } = req.body; // Перейменовано MovieId на movieId та HallId на hallId
            if (!movieId || !hallId || !startTime || !endTime || !date) {
                return next(ApiError.badRequest('All fields are required'));
            }
            const schedule = await Schedule.create({ movieId, hallId, startTime, endTime, date }); // Перейменовано MovieId на movieId та HallId на hallId
            return res.json(schedule);
        } catch (e) {
            return next(ApiError.internal(e.message));
        }
